...
和弦的构造是从根音开始，分别添加相应音程的音。下图是只区分相对位置的左手图，我以相对的数字代替绝对的音名，便于表述构造和弦的过程：1 代表根音，其他数字代表距离根音相对音程的音（不需要区分八度，共 12 个音）。由于左手有两列交错为大三度的单音，我分别将每列的根音置于中心，得到了两张图，便于观察。以下的讨论均为从此图构造的结果。
...
![](accordion_left_hand_major_chord.png)

观察此图，可以得出结论：大三和弦只适合根音在第二列时构造，因为在第一列时，包含关键的大三度音的键钮（浅黄色）离根音都太远。这个道理同样适用于所有大和弦，即以大三和弦（根音+大三度音+五度音）为基础的和弦，包括属七和弦、大七和弦、大九和弦等。

根音在第二列时，大三和弦可以按根音下的第三列“一键”实现，指法为 4 指按根音、3 指按大三和弦。大三度和五度音都在根音旁边，非常方便，可以在上述指法基础上，用 4 指按三音，2 指按五音。

在少数节奏欢快且通篇为大三和弦的曲子中，可以用 3 指按根音、2 指按大三和弦，目的是获得更多的手指力量。
...
手风琴上的分解和弦很少像钢琴一样完全分解成单音，而是组合单音键钮（第一、二列）和和弦键钮（第三到六列）。我以 `o` 代表单音，`x` 代表和弦，以下是各种节拍典型的实现方式：
- 2/4 拍：`|o x|o x|`；
- 3/4 拍：`|o x x|o x x|`；
...
以上 `o` 可以只用根音，也可以用其他和弦内音。如果用其他和弦内音与根音交替进行，就是所谓的交替低音（alternative bass）。
