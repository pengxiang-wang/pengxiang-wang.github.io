...
但是 LaTeX 的用法和 Word 很不一样，它不是在软件的上面点一点，而是在你草稿的纯文本里加一些特殊的标记符号，这些标记符号代表着格式，见下图（以下称加了这种标记的文本为 **LaTeX 代码**）。例如把“高数D论文”用 `\title{}` 和 `\textbf{}`括起来，就表示这一段字是标题（字体会自动变大并居中），而且要用加粗的样式。
