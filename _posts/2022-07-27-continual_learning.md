...
防止遗忘最直接的方式是在记忆中旧任务的训练数据，称为**重演**（replay 或 experience replay）数据。此时这块记忆通常称为情节记忆（**episodic memory**，模仿心理学术语）。重演数据不能太多，当然不允许使用全部的旧任务数据（否则就不是持续学习了），通常要求限制固定的重演数据记忆容量，需要精炼旧任务数据的信息。
