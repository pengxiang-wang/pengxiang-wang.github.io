...
- `plot_results.py`: 可视化实验结果的工具（见“处理实验结果”一节）；
...
- 数据集变量为 `data`：是一个嵌套字典，即字典的值还是字典
  ...
  - 第三层（39行）在数据集 'train', 'valid', 'test' 里面：
    ...
    - 标签 'y'：一个大 Tensor；
- 每个任务有几个类 `taskcla`（78行）：是一个列表，事实上对 Permuted MNIST，它固定是 `[10,...,10]`； 
- 输入维度 `size`：直接定义为常量 `[1,28,28]`（13行）。


从 `get` 函数参数可以看到，作者没有为用户提供什么选择，一个 Permuted MNIST 数据集基本是固定的，用户只能设置：
- 随机数种子 `seed`：控制 Permute 随机打乱操作的种子；
- `fixed_order`；
- `pc_valid`：验证集数据比例。
...
项目在 `networks` 文件夹的代码定义了网络结构，每个模型是一个 py 文件。每个文件都只定义了一个 `nn.Module` 名为 `Net` 的类。会写这些 `nn.Module` 类是深度学习的基础，请参考我的 [PyTorch 学习笔记]()。每个深度学习项目都大同小异，大都用到 MLP、AlexNet、ResNet 等网络，写法也差不多。
...
对于问题二，不可避免地要对每种网络结构衍生出一个修改版本，例如本项目中 mlp.py 衍生出 mlp_hat.py，alexnet.py 衍生出 alexnet_hat.py, alexnet_pathnet.py, alexnet_progressive.py, alexnet_lfl.py 等（后者都是涉及修改网络结构的 model-based 方法）。在使用这些 model-based 方法时，要求使用相应的网络结构。
...
此外还有一些细节。有 alexnet_hat.py 和 alexnet_hat_test.py 两个，它们区别在 43-50 行：是否对 task embedding 作归一初始化，与模型压缩实验有关。另外，有些带 hat 的 `Net` 类写了 `get_view_for` 函数，它是 HAT 算法的一个步骤写在了模型类里，在 HAT 的 train 函数调用，见下文。
...
- `train` 函数的核心工作在 40 行调用 `train_epoch` 函数，定义在 72 行，它的任务是训练任务 t 的一个 epoch。它的流程与普通的深度学习训练过程没什么区别，唯一要注意的是它是在训练任务 t（这个信息当做函数参数传入），体现在 88 行结果截取输出头 t；其他部分代码都在做一件事——动态调整学习率，这是深度学习的训练技巧，我放在后面的章节专门讨论。
...
## 处理实验结果
如果为空，21-22 行定义了默认的文件路径，可以看到是用`--experiment`、`--approach`等元信息命名的，用于区分不同实验的结果，


测试时把准确率存放到上三角矩阵 `acc` 中，t行u列表示训练完第t个任务时模型在任务u的准确率，这个是持续学习最主要的指标（见[持续学习基础知识笔记]()）；此外，在 178 行之后把一些结果信息用 pickle 保存下来（pickle 是一个 Python 内置库，可以完整保存、还原任意 Python 变量），在需要画图的时候被 `plot_results.py` 还原调用。

包括准确率结果、画图用的 pickle 文件，
由下文所述 `--output` 参数指定；

本项目是先把结果存下来，再在需要时单独对其可视化。可视化与核心业务分离，这个 py 文件就是单独的可视化程序。

## 实验细节

### 调参技巧

每一轮都在验证集上测试（调用 `eval` 函数）一下 loss，如果连续 lr_patience 个 epoch 验证集 loss 一直不下降，则把学习率调小一点：除以 lr_factor。 
...
随机数种子的作用  深度学习为了复现方便，公平，
需要把所有随机数设成一个种子，才是有效的。
...
比较固定，用户方便可调的东西不多，不过对于科研来说够了，又不是产品。不能设置数据集的。

不需要呈现给用户，不需要考虑太多的延展性问题，自己方便就行。例如 mlp_hat.py。实验需要什么，我就写什么，
