...
: 方法一
根音在第二列时，小三和弦可以按根音下的第四列“一键”实现：`C+C小`，指法为 `4+2`。如果这样按，就无法像大三和弦那样在此基础上按和弦内音，因为小三度音在下方较远的位置。

我们需要切换成指法 `3+2`，然后用 4 指去摸小三度音和它上方的五度音？？与大三和弦同理，3 指按根音、2 指按小三和弦的指法也常用于节奏欢快的曲子，获得更多的手指力量。对于小三和弦，这种指法需要把 2 指和 3 指伸展开来。

: 方法二
根音在第一列时，小三和弦无法“一键”实现，但仍然可以在上边一排方便地找到小三度音和五度音。
...
: 方法一
根音在第二列时，属七和弦可以按根音下的第五列“一键”实现：`C+C7`，指法为 `4+2`。如果这样按的话，无法在此基础上按和弦内音，因为小七度音位于根音向下 2 排的位置，能不能用指法 `3+2`？？
...
: 方法一
根音在第二列时，小七和弦可以直接在小三和弦上加小七度音实现：`C+C小+Bb`，指法为 `3+2+4`。

: 方法二
根音在第一列时，小七和弦可以通过“根音+以小三度音为根音的大三和弦”实现，即 `<u>C</u>+Eb大`，指法为 `4+2`。
...
: 方法一
根音在第一列时，小七和弦可以通过“根音+以小三度音为根音的小三和弦”实现，即 `<u>C</u>+Eb小`，指法为 `4+2`。
...
在手风琴上增和弦是很困难的，而减和弦相对容易很多。

增和弦由于是三和弦，只能用三个单音来实现。

减和弦不能直接一键实现，那是减七（省略 3 音）。但性质差不多。
...
以四音列
用四音列来理解，有利于记忆，因为大小调音阶大部分是，四音列+四音列的连接。
