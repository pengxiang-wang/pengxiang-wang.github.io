...
以 MNIST 为例，可以构造 Split MNIST、Permuted MNIST 两种数据集。Split MNIST 划分成（以 5 个任务为例）0v1, 2v3, 4v5, 6v7, 8v9；Permuted MNIST 每构造一个任务时先随机生成 $$\{1,\cdots,6000\}$$ 的下标集（permutation），然后在每个类别抽取此下标的数据组成任务（任务数随意，通常为 5 个）。 
